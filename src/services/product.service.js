...
const create = async (req) => {
  ...
  if (!req.file) {
    // kalau tidak ada foto yang ditambahkan
    throwError(400, {}, "mohon tambahkan gambar produk");
  }
  if (req.body.sku) {
    const uniqueSKU = await Product.findAll({
      where: {
        sku: req.body.sku,
      },
    });
    if (uniqueSKU) {
      throwError(400, {}, "SKU harus unik");
    }
  }
...
const getSummaryStock = async (req) => {
  ...
  return success(200, findProduct, "suskes dapatkan data");
};
