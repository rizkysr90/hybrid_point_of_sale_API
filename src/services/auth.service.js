...
const register = async (req) => {
  let { name, email, password, role, confirm_password, phone_number } =
    req.body;
...
const loginCustomer = async (req) => {
  ...
  const comparingPassword = await compare(
    password,
    findCustomerByEmail.password
  );
...
const registerCustomer = async (req) => {
  ...
  return success(
    201,
    { id: customerCreation.id, email: customerCreation.email },
    "sukses mendaftarkan customer"
  );
};
const login = async (req) => {
  ...
  return success(
    200,
    { id: findUserByEmail.id, role: findUserByEmail.role },
    "login berhasil"
  );
};
const refreshOtpReg = async (req) => {
  ...
  // UNIX Timestamp expired 5mnts after generate Date.now
  let exp_otp_reg = Date.now() + 300000;
  let userUpdate = await Customer.update(
    { otp_reg, exp_otp_reg },
    {
      where: {
        [Op.and]: [{ email }, { is_verified: 0 }, { id }],
...
const verifyNewCust = async (req) => {
  ...
  if (findUserByEmail.otp_reg === otp) {
    ...
    let verify = await Customer.update(
      { is_verified: 1, otp_reg: null, exp_otp_reg: null },
      {
        where: {
          [Op.and]: [{ email }, { is_verified: 0 }, { id }],
        },
...
const forgot_password = async (req) => {
  ...
  let token = jwt.sign({ id: findCust.id, email: findCust.email }, jwtSecret, {
    expiresIn: "1h",
  });
  let url =
    process.env.UI_HOST + "/forgot-password" + `/${findCust.id}` + `/${token}`;
  let emailContent = {
    from: `Rizki Plastik <${process.env.MAILER_EMAIL}>`,
    to: email,
...
const resetPassword = async (req) => {
  ...
  let hashedPassword = await hash(password);
  await Customer.update(
    { password: hashedPassword, otp_forgot_pass: null },
    {
      where: {
        [Op.and]: [{ id: userId }, { is_verified: 1 }],
      },
