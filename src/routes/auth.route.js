...
router.post('/login/users', 
    ...
    body('password').isLength({ min: 8 }).withMessage('Minimal panjang password 8 karakter'),
...
router.post('/register/customers', 
    ...
    body('email').isEmail().withMessage('email tidak valid').bail(),
    body('password').isLength({min : 8}).withMessage('minimal panjang password 8 karakter').bail(),
    validator,
    authController.registerCustomer);

router.post('/refreshOtp/registration',
    body('email').notEmpty().withMessage('email tidak boleh kosong').bail(),
...
router.post('/newCust/validation',
    body('email').notEmpty().withMessage('email tidak boleh kosong').bail(),
    body('id').notEmpty().withMessage('id tidak boleh kosong').bail(),
    body('otp').notEmpty().withMessage('otp tidak boleh kosong').bail(),
    body('email').isEmail().withMessage('email tidak valid').bail(),
    body('otp').isLength({ max: 4 }).withMessage('maksimal panjang otp 4 karakter').bail(),
    validator,
    authController.verifyNewCust
);  
router.post('/login/customers', 
    body('email').notEmpty().withMessage('email tidak boleh kosong').bail(),
    body('password').notEmpty().withMessage('password tidak boleh kosong').bail(),
    body('email').isEmail().withMessage('email tidak valid').bail(),
    validator,
...
router.get('/forgot-password/:userId/:token',
...
router.post('/forgot-password/:userId/:token',
    param('token').notEmpty().withMessage('token tidak boleh kosong').bail(),
    param('userId').notEmpty().withMessage('user id tidak boleh kosong').bail(),
    body('password').notEmpty().withMessage('password tidak boleh kosong').bail(),
    body('confirm_password').notEmpty().withMessage('password tidak boleh kosong').bail(),
    param('token').isJWT().withMessage('token tidak valid').bail(),
    param('userId').isNumeric().withMessage('user id tidak valid').bail(),
    validator,
